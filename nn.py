...
def process_data():
    ...
    if not os.path.isdir("./bitmaps/"):
        os.makedirs('./bitmaps')
    if not os.path.isdir("./splice_seeds/"):
        os.makedirs('./splice_seeds')
    if not os.path.isdir("./vari_seeds/"):
    ...
    if splice == 1:
        ...
        if round_cnt != 0:
            if round_cnt % 2 == 0:
                ...
                loss_value, grads_value = iterate([x])
                idx = np.flip(
                    np.argsort(np.absolute(grads_value), axis=1)[:, -MAX_FILE_SIZE:].reshape((MAX_FILE_SIZE,)), 0)
                val = np.sign(grads_value[0][idx])
